...
@Component({
  selector: 'app-scene',
  standalone: true,
  imports: [NgtArgs, NgtsEnvironment, NgtpEffectComposer, NgtpBloom],
  template: `
...
export class SceneComponent {
  ...
  // Semnale noi pentru mărimea și vizibilitatea spiralei pe Light Mode
  ...
  constructor() {
    ...
    for (let i = 0; i < constellationCount; i++) {
      ...
      constellationArray[i * 3] = r * Math.cos(theta);     // X
      constellationArray[i * 3 + 1] = y;                   // Y
      constellationArray[i * 3 + 2] = r * Math.sin(theta); // Z
    }
    ...
    effect(() => {
      const theme = AuthStore.theme(); // Citim tema
      ...
      if (theme === 'LIGHT' && this.hasEntered()) {
        ...
        // Puncte groase și vizibile pe Light Mode
        ...
        // Puncte fine și transparente pe Dark Mode
