    ...
    <header class="h-20 border-b border-white/10 flex items-center justify-between px-8 bg-transparent backdrop-blur-md">
      ...
      <div class="flex items-center gap-8">
        ...
        <button (click)="toggleTheme()" 
                class="relative flex items-center w-20 h-10 rounded-none border-2 transition-all duration-300 focus:outline-none overflow-hidden pointer-events-auto"
          ...
          <div class="absolute w-8 h-8 transition-transform duration-300 flex items-center justify-center"
        ...
        @if (isAuthenticated()) {
          <div class="flex items-center gap-3 relative group cursor-pointer pointer-events-auto">
            ...
            <div class="w-10 h-10 bg-[#0ABAB5] text-black flex items-center justify-center font-bold text-lg shadow-[0_0_15px_rgba(10,186,181,0.5)]">
              {{ user()?.name?.charAt(0) }}
            </div>
            
            <!-- Dropdown -->
            <div class="absolute right-0 top-full mt-4 w-56 bg-black border border-[#0ABAB5]/30 hidden group-hover:block shadow-[0_10px_30px_rgba(10,186,181,0.2)] z-50">
          ...
          <a routerLink="/auth/login" class="px-6 py-2 border-2 border-[#0ABAB5] text-[#0ABAB5] hover:bg-[#0ABAB5] hover:text-black transition-all text-sm font-bold uppercase tracking-widest shadow-[0_0_15px_rgba(10,186,181,0.3)] hover:shadow-[0_0_25px_rgba(10,186,181,0.6)] pointer-events-auto">Login</a>
        }
      </div>
    </header>
  `
